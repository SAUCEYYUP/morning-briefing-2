...
def get_groq_analysis(prices, fg_value, fg_label, macro,
    ...
    for coin_id, symbol in CRYPTO_WATCHLIST.items():
        ...
        if c is not None:
            cs = ("+{:.2f}%".format(c) if c >= 0 else "{:.2f}%".format(c))
        else:
            cs = "N/A"
    ...
    for label, d in macro.items():
        ...
        if val and val != ".":
            try:
                ...
                macro_lines.append(
                    label + ": {:.2f} ({:+.2f} vs prev) as of ".format(float(val), diff) + str(date)
                )
            except Exception:
    ...
    for m in poly:
        if m["yes_prob"]:
            poly_lines.append(
                "- " + m["question"][:80] +
                " | YES: {:.0f}%".format(m["yes_prob"]) +
                " | Vol: ${:.1f}M".format(m["volume"] / 1e6)
            )
    if not poly_lines:
        poly_lines = ["No data"]
    ...
    prompt = (
        ...
        "First write exactly 3 lines (no extra text before them):\n"
...
def build_message(prices, fg_value, fg_label, macro, poly, calendar,
    ...
    lines.append("\U0001f4c5  <i>" + now + "  \u00b7  Singapore  \u00b7  6 AM</i>")
    ...
    sub_parts = []
    if btc_dom is not None:
        sub_parts.append("BTC Dom: <b>" + str(btc_dom) + "%</b>")
    if funding_rate is not None:
        if abs(funding_rate) < 0.005:
            ...
            fr_label = "Fear"
        fr_str = "{:+.4f}".format(funding_rate)
        sub_parts.append("Funding: <b>" + fr_str + "%</b> <i>(" + fr_label + ")</i>")
    if ls_ratio is not None:
        sub_parts.append("L/S: <b>" + "{:.2f}".format(ls_ratio) + "</b>")
    if sub_parts:
    ...
    lines.append(DIVIDER)
    lines.append("\U0001f4c9  <b>US MACRO DATA</b>")
    lines.append(SPACE)
    new_releases = []
    for label, d in macro.items():
        val  = d["value"]
        ...
        lines.append(
            "  " + fresh + "<b>" + label + "</b>:  " + vs + chg + "  <i>(" + str(date) + ")</i>"
        )
    lines.append(SPACE)
    if new_releases:
    ...
    if poly:
        for m in poly:
            if m["volume"] >= 1e6:
                ...
                vol = "${:,.0f}".format(m["volume"])
            yes = "  <b>YES " + "{:.0f}".format(m["yes_prob"]) + "%</b>" if m["yes_prob"] else ""
    ...
    lines.append(DIVIDER)
    lines.append("\U0001f9e0  <b>DRUCKENMILLER ANALYSIS</b>")
    lines.append(SPACE)
    clean_ai = []
    skip = False
    for line in ai_text.split("\n"):
    ...
    lines.append(DIVIDER)
    lines.append("\U0001f916  <i>CoinGecko \u00b7 Binance \u00b7 FRED \u00b7 Polymarket \u00b7 Groq AI</i>")
    return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    macro_news         = get_news(
        "Federal Reserve OR CPI OR inflation OR GDP OR recession OR FOMC OR ISM PMI", 5
    )
    crypto_news        = get_news(
        "bitcoin OR ethereum OR crypto OR DeFi OR polymarket OR hyperliquid", 5
    )
